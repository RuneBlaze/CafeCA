...
namespace Cafeo.UI
{
    ...
    {
        [InfoBox("Use Shift + $num to run preset commands.")] [SerializeField]
        ...
        private void Start()
        {
            ...
            {
                foreach (var ally in Scene.Allies()) ally.Revive();
            });
            ...
            RegisterCommand("addEffect",
                ...
                objects =>
                {
                    ...
                    var userData = new Vector4((float)objects[1], (float)objects[2], (float)objects[3],
                        (float)objects[4]);
        ...
        private void OnText(string text)
        {
            if (text.Contains(";"))
            {
                ...
                int repeats;
                var success = int.TryParse(modifiedTokens[0], out repeats);
                Assert.IsTrue(success);
                for (var i = 0; i < repeats; i++) OnText(modifiedTokens[1]);
                return;
            }
            ...
            {
                var modifiedTokens = text.Split("&").Select(it => it.Trim()).ToArray();
                foreach (var token in modifiedTokens) OnText(token);
                return;
            }
